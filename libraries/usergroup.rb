...
class Chef
  class Resource::FortigateUsergroup < Resource::LWRPBase
    resource_name :fortigate_usergroup

    # Set the resource name
    self.resource_name = :fortigate_usergroup
    ...
    attribute :member, kind_of: [String, Array], default: nil
...
class Chef
  class Provider::FortigateUsergroup < Provider::LWRPBase
    include Fortigate::Helper

    def load_current_resource
      @current_resource ||= Resource::FortigateUsergroup.new(new_resource.name)
      ...
      if not c.nil?
        ...
        @current_resource.member(c['member'])
      end
